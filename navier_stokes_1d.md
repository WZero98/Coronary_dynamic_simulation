...
\[
F = \begin{bmatrix} Q \\ \dfrac{\alpha Q^2}{A} + \dfrac{p_{\mathrm{mmHg}}(A)\,A}{\rho^*} \end{bmatrix},
\quad
...
| \(Q\) | 体积流量（沿 \(x\) 正向为出口方向） |
| \(\rho\) | 血液物理密度 (g/cm³) |
| \(\rho^*\) | 动量通量/波速中与 mmHg 制压力配对的**有效密度**，\(\rho^* = \rho / 1333.22\) |
| \(\alpha\) | 动量修正系数（层流常取 1.1） |
| \(\mu\) | 动力粘度相关项（代码中为 `mu`，cm²/s 量级） |
| 摩擦项 | Poiseuille 型 \(-8\pi\mu Q/A\) |

**单位约定**：tube law 与后处理中 \(p\)、\(\beta\) 一律用 **mmHg**；守恒方程动量通量中**不再**在通量函数内把 \(p\) 换成 dyne/cm²，而是使用 \(\rho^*\) 使 \(p_{\mathrm{mmHg}} A / \rho^*\) 与 CGS 形式 \(p_{\mathrm{dyne}} A / \rho\) 数值等价。换算在 `BloodFlowParameters.momentum_rho()`（即 `coronary_constants.rho_for_mmhg_pressure_coupling`）中**一次性**完成。
...
小扰动压力波速（\(p\)、\(\beta\) 均为 mmHg 制）：

\[
c(A) = \sqrt{\frac{2\,\beta\,\sqrt{A}}{\rho^*}}
\]

与将 \(\beta\) 换为 cgs 后使用 \(c = \sqrt{2\beta_{\mathrm{cgs}}\sqrt{A}/\rho}\) 等价。
...
| 物理密度 \(\rho\) | g/cm³（默认 1.05） |
| 动量耦合 \(\rho^*\) | g/cm³ / (1333.22)，由 \(\rho\) 导出 |
| 粘度 \(\mu\) | cm²/s |
| 压力 \(p\)、tube law 的 \(\beta\) | mmHg、mmHg/√cm |
| 时间 \(t\) | s |

`coronary_constants.MMHG_TO_DYNE_PER_CM2` 仅用于定义 \(\rho^*\)，**不出现在** `_flux_vector` / `_wave_speed` 等离散算子内部。
...
|------|------|------|
| `rho` | 1.05 | 物理密度 (g/cm³) |
| `momentum_rho()` | — | 动量通量/波速用 \(\rho^* = \rho/1333.22\) |
| `alpha` | 1.1 | 动量修正系数 |
| `nu` | 0.0035 | 运动粘度 (cm²/s) |
| `cfl` | 0.5 | CFL 数 |
